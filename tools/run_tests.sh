...
if uname|grep -q -i 'CYGWIN' && test -x "Release-2005/re2c.exe"; then
re2c='Release-2005/re2c.exe'
else
re2c='./src/re2c'
fi
if test ! -x "${re2c}"; then
...
re2c="${valgrind}${re2c}"
echo "Testing: ${re2c}"
if ${re2c} -v|grep -q 're2c 0.13.6 *\r*'; then
echo "Version: 0.13.6"
else
echo "Warning: test build for 0.13.6 but testing `${re2c} -v`"
fi
echo "Diffing: ${diff}"
if test $# = 0; then
tests=`for i in tests lessons; do find ./$i -name '*.re'; done|sort`
else
tests="$@"
fi;
for x in $tests; do
	...
	genname=`echo $switches|sed -e 's,--.*$,,g' -e 's,^.[^o]*$,,g' -e 's,^[^ot]*t.*o.*$,,g' -e 's,^-[^o]*o\(.*\),./tests/\1,g'`
	headers=`echo $switches|sed -e 's,--.*$,,g' -e 's,^.[^t]*$,,g' -e 's,^[^ot]*o.*t.*$,,g' -e 's,^-[^t]*t\(.*\),./tests/\1,g'`
	switches=`echo $switches|sed -e 's,^-\([^ot-]*[ot]\)\(.*\)$,-\1./tests/\2,g'`
	# don't use the -o flag, since it makes it harder to diff.
	echo $x: $switches
	outname=./tests/`basename ${x%.re}.c.temp`
	outdiff=./tests/`basename ${x%.re}.c.diff`
	typname=./tests/`basename ${x%.re}.h.temp`
	typdiff=./tests/`basename ${x%.re}.h.diff`
	$re2c $switches $x 2>&1 | sed -e "s,$x,`basename $x`,g" -e "s,/\* Generated by re2c .*\*/,/\* Generated by re2c \*/,g" > $outname
	if test -n "$genname"; then
		cat $genname | sed -e 's,./tests/,,g' -e "s,/\* Generated by re2c .*\*/,/\* Generated by re2c \*/,g" > $outname
		rm $genname
	fi
	ok=1
	if test -n "$headers"; then
		cat $headers | sed -e 's,./tests/,,g' -e "s,/\* Generated by re2c .*\*/,/\* Generated by re2c \*/,g" > $typname
		rm $headers
